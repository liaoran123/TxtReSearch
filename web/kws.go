...
// 打开txt文件
func Kws(w http.ResponseWriter, req *http.Request) {
	pubgo.Tj.Brows("kws")
	rd := new(kws)
	...
	rd.getkws(intp) //string(filetext)
	//页码列表
	...
	t, _ := template.New("kws.html").Funcs(funcMap).ParseFiles(TemplatesFiles...)
	//--New("index.html") 的 index.html必须是TemplatesFiles第一个文件名

	err = t.ExecuteTemplate(w, "kws.html", rd)
	if err != nil {
